...
ROUTES: tuple[Route, ...] = (
    ...
    Route(
        name="tts-custom",
        reference="references/tts-custom.md",
        reason="Custom or fine-tuned TTS model packaging or deployment request.",
        patterns=(
            r"\bnemo2riva\b",
            r"\briva-build\b",
            r"\briva-deploy\b",
            ...
            r"\.riva\b",
            r"\bfine[- ]tuned\b",
            r"\bcustom (?:tts|voice)\b",
            r"\btts\b",
            r"\btext[- ]to[- ]speech\b",
            r"\bspeech synthesis\b",
        ),
        next_steps=(
            "Read references/tts-custom.md.",
            "Verify the exact base image, model family, and riva-build inline block from current NVIDIA TTS docs.",
            "Keep commands and artifacts named Riva unless upstream docs say otherwise.",
        ),
    ),
    Route(
    ...
    Route(
        name="tts-pipelines",
        reference="references/tts-pipelines.md",
        reason="TTS synthesis pipeline configuration, runtime features, or advanced synthesis options request.",
        patterns=(
            r"\bssml\b",
            r"\bzero[- ]shot\b",
            r"\bvoice clon\w*\b",
            r"\bpronunciat\w*\b",
            r"\bcustom.?dict(?:ionary)?\b",
            r"\bexaggeration\b",
            r"\bcustom_configuration\b",
            r"\blpcm\b",
            r"\bogg.?opus\b",
            ...
            r"\bsample rate\b",
            r"\btts.*stream\w*\b",
            r"\bstream\w*.*synthesis\b",
            r"\bemotional (?:style|tts)\b",
            r"\btts\b",
        ),
        next_steps=(
            "Read references/tts-pipelines.md.",
            "Separate runtime-only options (zero-shot, SSML, custom_configuration) from build-time options that require riva-build.",
            "Verify custom_configuration key names and SSML support on the current TTS customization docs.",
        ),
    ),
    Route(
        name="tts-pronunciation",
        reference="references/tts-pronunciation.md",
        reason="TTS pronunciation discovery, IPA candidate generation, or custom dictionary construction request.",
        patterns=(
            r"\bphoneme\b",
            r"\bipa\b",
            r"\bpronunc(?:iation|e|ing)?\b",
            r"\bmispronounce\w*\b",
            r"\bsounds? (?:wrong|off|bad|correct|right)\b",
            r"\baccent\b",
            r"\bg2p\b",
            r"\bgrapheme\b",
            r"\bvariant[_\s][abc]\.wav\b",
            r"\btts\b",
        ),
        next_steps=(
            "Read references/tts-pronunciation.md.",
            "Generate 2-3 IPA candidates using linguistic knowledge; do not use phone symbols outside the model's supported set.",
            "Synthesize variant_a.wav, variant_b.wav, etc. using the inline script; wait for user choice before outputting final formats.",
            "Output the chosen pronunciation in all three formats: custom_dictionary wire string, SSML <phoneme> tag, and session dict file.",
            "Verify SSML <phoneme> support on the TTS customization page before recommending SSML as the delivery format.",
        ),
    ),
    Route(
        name="readiness",
